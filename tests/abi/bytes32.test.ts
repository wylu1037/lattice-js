import { decodeBytes32String, encodeBytes32String } from "@/utils/index";
...
describe("encodeBytes32String", () => {
  ...
  it("correctly encodes string with maximum length", () => {
    ...
    expect(hexlify(result)).toBe(
      "0x6162636465666768696a6b6c6d6e6f707172737475767778797a313233343500"
    );
  });

  it("throws error for string too long", () => {
    expect(() =>
      encodeBytes32String("abcdefghijklmnopqrstuvwxyz123456")
    ).toThrow("bytes32 string must be less than 32 bytes");
  });
});
...
describe("decodeBytes32String", () => {
  ...
  it("throws error for invalid length bytes", () => {
    ...
    expect(() => decodeBytes32String(encoded)).toThrow(
  ...
  it("throws error for missing null terminator", () => {
    const encoded =
      "0x6162636465666768696a6b6c6d6e6f707172737475767778797a3132333435ff";
    expect(() => decodeBytes32String(encoded)).toThrow(
